...
const onMousemove = (event: any) => {
  ...
  if (cursor.value === "move") {
    ...
    selectItem.value.rotate =
      selectItem.value._rotate + angleAfter - angleBefore;
  }
...
const onUploadChange = (uploadFile: any) => {
  ...
  // 新增图片初始位置
  const offsetTop =
    imagesList.value.length > 0 ? imagesList.value[0].top + 50 : 0;
  const offsetLeft =
    imagesList.value.length > 0 ? imagesList.value[0].left + 50 : 0;
